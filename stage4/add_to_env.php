...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($businessName) && !empty($langCode) && !empty($country) && !empty($city) && !empty($street) && !empty($houseNumber)) {
        ...
        if ($envFile) {
            ...
            $logMessage = "[" . date("Y-m-d H:i:s") . "] [STAGE4] ✅ ENV file update successful! Gym name set: $businessName, Language: $langCode, Country: $country, City: $city, Street: $street, House Number: $houseNumber\n";
            file_put_contents("../LOG.log", $logMessage, FILE_APPEND);

            header("Location: ../stage5");
            exit();
        } else {
            $logMessage = "[" . date("Y-m-d H:i:s") . "] [STAGE4] ❌ Failed to open .env file for writing!\n";
            file_put_contents("../LOG.log", $logMessage, FILE_APPEND);
