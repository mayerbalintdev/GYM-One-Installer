...
if (!$conn) {
    ...
    $logMessage = "[" . date("Y-m-d H:i:s") . "] [STAGE5] ✅ Successful database connection to the database: $db_name on host: $db_host\n";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['install'])) {
    ...
    if (file_exists($sqlFile)) {
        ...
        if (!isset($response['status'])) {
            ...
            $logMessage = "[" . date("Y-m-d H:i:s") . "] [STAGE5] ✅ The installation was successful, the SQL commands ran.\n";
            file_put_contents("../LOG.log", $logMessage, FILE_APPEND);
        }
    } else {
        $response['status'] = 'error';
        $response['message'] = 'The installer.sql file cannot be found.';
        $logMessage = "[" . date("Y-m-d H:i:s") . "] [STAGE5] ❌ The installer.sql file cannot be found.\n";
        file_put_contents("../LOG.log", $logMessage, FILE_APPEND);
    }
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="text-center mt-4">
                            <button id="installBtn" class="btn btn-primary"><?php echo $translations["sql-upload-btn"]; ?></button>
                        </div>
                        <div class="mt-4">
                            ...
                            <div class="text-center mt-4">
                                <a href="../stage6/" id="continueButton" class="btn btn-success"><?php echo $translations["next"]; ?></a>
                            </div>
                        </div>
                    </div>
                </div>
